...
namespace Tkach.Networks.Lab3_4
{
	...
	{
		struct icmphdr hdr;
		char message[ICMP_Requester.PING_PKT_S - icmphdr.Size()];
	};

	class ICMP_Requester
	{
		// define the packet constants
		public static int PING_PKT_S = 32;            /* ping packet size */
		public static int PORT_NO = 0;            /* automatic port number */
		public static int PING_SLEEP_RATE = 1000000;  /*  ping sleep rate */
		public static int RECIEVE_TIMEOUT = 1;         /*  timeout delay for receiving packets in secs  */

		// define the Ping Loop to maintain infinite pinging
		int pingLoop = 1;

		// interrupt handler
		public void interruptHandler(int var)
		{
			pingLoop = 0;
		}

		// calculate the check sum
		public int checksum(int [] buffer, int length)
		{
			int[] buff = buffer;
			int sum = 0;
			short result;
...
} else
{
	...
	if (flag)
	{
		...
		{
			Console.Writeline("Error...Packet received with ICMP type ")pckt.hdr.type
			Console.Writeline(" code " << to_string(pckt.hdr.code))endl;
		}
		else
		{
